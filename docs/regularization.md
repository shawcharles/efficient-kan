...
The optional entropy term is computed from the normalised `l1_like` values:
...
| `regularize_entropy` | Weight on the entropy term over normalised spline-weight magnitudes. |

Both arguments are multipliers on the returned penalty components. They are not
optimiser weight decay values.

## Efficient Proxy vs Activation-Based KAN Regularisation
...
`efficient-kan` does not do that. Its regularisation is based on spline weights,
...
- no need to retain large activation tensors for regularisation;
...
activation-based regularisation from the original KAN paper or implementations
that explicitly store expanded activations.

For statistical workflows, treat the regularisation choice as part of the model
...
## Interaction with Optimiser Weight Decay

`regularization_loss()` and optimiser weight decay are different mechanisms.
...
AdamW weight decay acts through the optimiser update. `regularization_loss()`
adds a differentiable scalar term to the training objective. You can use either
or both, but the combination changes the effective regularisation policy and
should be documented in research artefacts.
...
- Remember that this is spline-weight regularisation, not expanded
  activation-based KAN regularisation.
