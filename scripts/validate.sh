...
with tarfile.open(sdist) as archive:
    ...
    assert any(name.endswith("docs/Gemfile") for name in names)
    assert any(name.endswith("docs/index.md") for name in names)
    assert any(name.endswith("docs/_config.yml") for name in names)
    assert any(name.endswith("docs/_sass/color_schemes/efficient-kan.scss") for name in names)
    assert any(name.endswith("docs/_sass/custom/setup.scss") for name in names)
    assert any(name.endswith("docs/_sass/custom/custom.scss") for name in names)
    assert any(name.endswith("docs/_includes/footer_custom.html") for name in names)
    assert any(name.endswith("docs/_includes/nav_footer_custom.html") for name in names)
    assert any(name.endswith("docs/assets/images/logo.svg") for name in names)
    assert any(name.endswith("docs/assets/images/favicon.svg") for name in names)
    assert any(name.endswith("docs/third-party-notices.md") for name in names)
